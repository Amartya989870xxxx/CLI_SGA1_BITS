...
This command created a new directory named `Question2` to store all the files related to Question 2. It helps organize the project files separately from other questions.
...
This command changed the current working directory to `Question2`. All subsequent files and directories for this task were created inside this folder.
...
This command verified that the permissions were successfully updated. The output changed from `-rw-rw-r--` (or similar) to `-rw-------`, confirming that only the owner has access.
